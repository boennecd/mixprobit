...
the relative error is below `releps`. Since GHQ is deterministic, we use 
a number of nodes such that this number of nodes or `streak_length` less 
(which is set to three at the time of this writing) 
value of nodes with GHQ gives a relative error which is 
below the threshold. We use a minimum of 10 nodes at the time of 
this writing.
...
the average computation time, then we show the mean MSE, and we end by 
looking at the number of nodes that we need to use with GHQ. The latter 
...
The computation time is in `r comp_time_mult`s of a second. The mean MSE is 
