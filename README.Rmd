...
```{r pre_mult_quick_ex, echo = FALSE}
cur_vars <- ls()
```
...
The CDF approach is noticeably faster. 
One explanation is that the AGHQ we are 
using, as of this writing, for the integrand with other methods uses 
$8(c - 1)$ evaluations of the standard normal distribution's CDF with $c$
being the number of categories per observation in the cluster plus the 
$K$ evaluations of the inverse normal CDF, and the 
overhead of finding the mode. 
In the example above, this means that we do 
`8 * n * (p - 1)`, `r 8 * n * (p - 1)`, CDF evaluations for each of the 
`maxpts`, `r maxpts`, evaluations. We show how long this takes to compute 
below when the evaluations points are drawn from the standard normal 
distribution

```{r pnorm_is_slow, cache = 1}
local({
  Rcpp::sourceCpp(code = "
...
In contrast, the CDF approximation can be implemented with only 
`n * p`  evaluation of the CDF, `n (p - 1)` evulations of the inverse CDF 
and `n * (p - 1) - 1`, `r n * (p - 1) - 1`, evaluations of the log of the 
standard normal distribution's PDF for each of `maxpts`, `r maxpts`, 
evaluations. This is much faster to evaluate as shown below

```{r pdf_is_fast, cache = 1}
local({
  u <- rnorm(n * p * maxpts)
  Rcpp::sourceCpp(code = "
    // [[Rcpp::depends(RcppArmadillo)]]
...
```{r clean_mult_quick_ex, echo = FALSE}
rm(list = setdiff(ls(), cur_vars))
```

## More Rigorous Comparison (Multinomial)

```{r mult_default_params, cache = 1, echo = FALSE}
# default parameters
ex_params <- list(
  streak_length = 4L, 
  max_b = 25L, 
...
Again, we perform a more rigorous comparison. 
We fix the relative error of the methods before hand such that 
the relative error is below `releps`, $`r ex_params$releps`$. 
Ground truth is computed with brute force MC using `n_brute`, 
$`r ex_params$n_brute`$, samples.
...
```{r mult_show_default_params, ref.label = "mult_default_params", eval = FALSE}
```

```{r mult_def_sim_experiment, cache = 1, dependson = "mult_default_params"}
# perform a simulations run for a given number of observations and random
# effects. First we fix the relative error of each method such that it is
# below a given threshold. Then we run each method a number of times to 
# measure the computation time. 
...
```{r mult_show_sim_experiment, cache = 1, dependson = "mult_def_sim_experiment"}
print.sim_experiment <- function(x, ...){
  old <- options()
  on.exit(options(old))
  options(digits = 6, scipen = 999)
...
```{r mult_run_larger_sim_ex, message = FALSE, warning = FALSE}
# number of observations in the cluster
n_vals <- 2^(1:5)
# number of random effects
p_vals <- 3:6
...
```{r mult_meta_create_table, ref.label = "meta_create_table", eval = TRUE}
```

```{r mult_create_table, ref.label = "create_table", eval = TRUE, results = "asis"}
```
...
We plot the average evaluation time and error below using a different number 
of nodes for different cluster sizes, number of clusters and random 
effects.

```{r show_rig_aprx_mult_inner_res}
